...
### 5. 多段影音精準對齊與口型同步原則 (Stitching Sync Defense)
* 嚴格防範「斷頭句」與「無聲畫面」：在處理講者多次 NG、口吃或停頓的素材時，禁止將單句字幕掛在「過短或殘缺」的視訊片段上。
* 你必須遵循 **「複合取樣縫合」** 規則：找出所有包含有效發音的原始影格段落，將它們以 list 的形式無縫 append 到時間軸，並將字幕的顯示區間與這些拼接段落的總時長進行比例綁定。這能保證聽覺流暢度的同時，視覺上絕不丟失任何對應的說話畫面。
